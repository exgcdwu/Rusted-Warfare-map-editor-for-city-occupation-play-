...
`每一条均需要顺序执行，开头不得出现空格。请尽量复制粘贴后执行。`
    ...
    pip install rwmapeditor-exgcdwu==1.7.0 --no-deps
...
城夺地图的核心是建筑刷新。让我们以建筑刷新为例，开始了解自动化程序是如何运行的。
...
`对于普通宾语来说，我们约定，开头为宾语的类型，后面还会跟上它们的属性。不要把双引号也加上。"//"是注释符号，后面的文本请无视。`
...
请查看[宾语自动化参数说明](https://github.com/exgcdwu/Rusted-Warfare-map-editor-for-city-occupation-play-/blob/main/auto/readme/auto_tutorial.md)的inadd_info到building_info的内容。可以看到很多参数可供info选择。还有info宾语默认参数这部分内容，会对许多共同参数进行解释。该文档里包含当前版本中宾语自动化的所有参数和介绍，最为全面完整。

此外，还有一份[宾语自动化示例](https://github.com/exgcdwu/Rusted-Warfare-map-editor-for-city-occupation-play-/blob/main/auto/readme/example/auto_example.tmx)，这是一份地图文件，可供实际作图是复制粘贴和参考。

接下来，我们将具体讲解。

我们可以先查看building_info的内容。
...
`对于自动化宾语来说，我们约定，info打头为info宾语，标记打头为标记宾语，后面紧跟它们的名字，类型不填。如果是info宾语，后面还会跟上它们的属性。不要把双引号也加上。"//"是注释符号，后面的文本请无视。`
...
有关于队伍检测宾语自动化的info是teamDetect_info。下面是该info的详细参数，您也可以在[宾语自动化参数说明](https://github.com/exgcdwu/Rusted-Warfare-map-editor-for-city-occupation-play-/blob/main/auto/readme/auto_tutorial.md)中的teamDetect部分看到。
...
除了许多功能性的info，还有专门的info用于引用。它就是dictionary_info。下面是该info的详细参数，您也可以在[宾语自动化参数说明](https://github.com/exgcdwu/Rusted-Warfare-map-editor-for-city-occupation-play-/blob/main/auto/readme/auto_tutorial.md)中的dictionary部分看到。
...
现在，我们学会了队伍检测的自动化和引用如何使用。现在你可以更方便的检测你的建筑的归属，然后将队伍检测id用于控制其他各种宾语，确定地图的胜负判定方式，并且提高地图的可玩性。学会这些，你已经完全可以利用以上知识来大幅减轻您的工作量了。目前的城夺地图，您已经都可以去做了。然而，我们还有更灵活的宾语自动化处理方式。
...
然而，仍然是有二者一起使用的需求的。想必作者不愿意放置一个城市的时候要同时放两个标记宾语。我们希望让自动化在具体放置上更简便。这样的话，tree_info就派上用场了。以下是tree_info的参数，您也可以在[宾语自动化参数说明](https://github.com/exgcdwu/Rusted-Warfare-map-editor-for-city-occupation-play-/blob/main/auto/readme/auto_tutorial.md)中的tree部分看到。
...
我们像是搭积木一样，把这样一个建筑+队伍检测的宾语自动化搭出来了。然而这些积木块的功能还是比较完整的。我们能不能从头搭一个自动化模式呢？这就需要object_info了。object_info的参数如下，您也可以在[宾语自动化参数说明](https://github.com/exgcdwu/Rusted-Warfare-map-editor-for-city-occupation-play-/blob/main/auto/readme/auto_tutorial.md)中的object部分看到。
...
我们发现，这个莫斯科城本身也有城名，可能也需要进行一些修改。让我们先来看multiText是如何实现的吧。multiText_info的参数如下。您也可以在[宾语自动化参数说明](https://github.com/exgcdwu/Rusted-Warfare-map-editor-for-city-occupation-play-/blob/main/auto/readme/auto_tutorial.md)中的multiText部分看到。
