...
inunitAddoffset: 当isinadd为是时可选，数字数组，默认为"-10 0"，建筑初始添加宾语偏移。

inunitAddoffsetsize: 当isinadd为是时可选，数字数组，默认为"20 0"，建筑初始添加宾语大小改变。
...
isteamText：可选，默认为否，在teamText_info中默认为是，是否有队伍文本显示。
