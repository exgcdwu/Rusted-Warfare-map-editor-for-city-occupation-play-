...
- [宾语自动化参数说明](#宾语自动化参数说明)
  ...
  - [building\_f\_info](#building_f_info)
...
args: 声明标记宾语的必填参数。比如"unit,str;num,str"，这一串表明标记宾语必须填入2个参数，第一个是代入unit，第二个代入num。参数名后面目前支持str和bool格式。bool格式下，可以输入"true"或"false"。标记宾语在前缀之后添加supplyDepot.1意思是unit为supplyDepot，num为1。如果info宾语的prefix为e，那么标记宾语将填esupplyDepot.1。如果isprefixseg标记了，那么标记宾语将填e.supplyDepot.1。
...
如果格式为bool。那么，不需要额外输入参数，认为是"true"。
...
inaddwarmup：当isinadd为是时可选，默认为{addWarmup}，表示单位添加的warmup。
...
与building_info绝大部分参数均相同。区别是，其中的建筑检测将会抑制建筑刷新，而不是激活建筑刷新。因此，building_f_info有必要使用振荡器（[振荡器](#flash_info)）帮助刷新。将振荡器的id加入deacti，即可按照振荡器顺序试图刷新建筑。介于这个基本差别，默认参数有所改变。
现在minUnits默认为1而maxUnits不存在（它们均可选）。
...
这是一个产生多个单位添加宾语的info宾语。类似于multiText。
...
这是一个振荡器，为了解决unitAdd在deactivatedBy由激活到不激活时，异常触发刷兵的bug。该振荡器需要一片空间，可以作为控制台一部分。具体来说，将振荡器检测id加入unitAdd的deactivatedBy中，就能在振荡器激活变为非激活时，直接产生刷新。
...
idprefix: 表示振荡器输出的id前缀，城市id在idprefix后将自动按照1,2...顺序延伸，请确保其他id没有此前缀。

initialtime：必选，字符串数组。检测id停止激活的若干初始时间。最后一个initial为周期开始的时间。*时间必须以s结尾*
...
periodtime：可选，字符串数组，默认没有。检测id停止激活的若干周期时间（在周期中停止激活的相位）。最后一个periodtime为周期时间。*时间必须以s结尾*
...
### 安装
    ...
    pip install rwmapeditor-exgcdwu==1.6.4
