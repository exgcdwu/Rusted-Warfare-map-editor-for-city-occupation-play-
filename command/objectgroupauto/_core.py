...
def auto_func(args = None):
    ...
    for key, info in info_now.items():
        for tobject in map_now.iterator_object_s(default_re = {rw.const.OBJECTDE.type: r"(?!.+)", 
            ...
            if bool(re.match(key, info_name)):
                ...
                standard_out(language, isverbose == 2, "Info object information is being output..." + 
                             "|info宾语信息正在输出...")
                if isverbose == 2:
                    ...
                    standard_out(language, isverbose == 2, temp_pri)
    ...
    while index_tobject < len(tobject_list):
        ...
        if ischange:
            ...
            standard_out(language, isverbose == 2 and (not(isdelete_sym or isdelete_all or isdelete_all_sym)), 
            ...
            standard_out(language, isverbose == 2 and (isdelete_all or isdelete_all_sym), 
            ...
            standard_out(language, isverbose == 2 and (isdelete_sym and(not (isdelete_all or isdelete_all_sym))), 
    ...
    standard_out(language, isverbose == 2, f"Tagged objects time with different info:|不同info标记宾语的运行时间:")
    ...
    standard_out(language, isverbose == 2, langstr_time)

    standard_out(language, isverbose == 2 and isdebug, f"Tagged objects time with different info(operation):|不同info标记宾语的运行时间(operation):")
    ...
    langstr_time = langstrlist_add(langstr_time_list)
    langstr_time = "|".join(["(" + langtime[:-2] + ")" for langtime in langstr_time.split("|")])
    standard_out(language, isverbose == 2 and isdebug, langstr_time)

    standard_out(language, isverbose == 2, f"Tagged objects time with different prefix:|不同前缀标记宾语的运行时间:")
    ...
    standard_out(language, isverbose == 2 and isdebug, f"Tagged objects time with different prefix(operation):|不同前缀标记宾语的运行时间(operation):")
    ...
    standard_out(language, isverbose == 2 and isdebug, f"Tagged objects time with different operation:|不同operation的运行时间(标记宾语):")
