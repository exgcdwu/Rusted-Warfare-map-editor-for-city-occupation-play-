...
cite_name：标记宾语的引用，如果没有，无法被引用。每个带 cite_name 的宾语都会自动提供常用几何引用：cite_name.tx/ty/tw/th（原始）与 cite_name.nx/ny/nw/nh（移动后），详见“坐标/大小设置（所有 info 通用）”。比如在某info宾语中设置args为"cite_name,str"，在使用该info宾语格式的标记宾语中名称为"e.td"，其中e是前缀。在cite_name后面加"."再添加属性名称，即可实现引用。比如td.aunit表明将会引用该标记宾语的其中aunit的内容。
...
**数字/布尔数组的元素允许是引用表达式**：读取参数时不会强制要求每一项都是纯数字/布尔；如果某个元素是引用（如 `{sx}`、`{rx}`），会先保留为字符串，等实际使用时再翻译成数字/布尔。能翻译成功则正常使用，否则报错。例如 `gset="{sx} 0"` 中的 `{sx}` 会在计算产生宾语坐标时被解析。
...
所有 info 都支持以下通用几何属性，作用于**标记宾语自身**：

- `sx`、`sy`、`sw`、`sh`：直接设置标记宾语自身的坐标和大小（绝对）。不设置时保持原始值。
- `ox`、`oy`、`ow`、`oh`：在绝对设置（或当前坐标/大小）基础上对标记宾语自身继续偏移。不设置时为 0。
- 顺序：先应用 `sx/sy/sw/sh`，再应用 `ox/oy/ow/oh`，得到标记宾语移动后的坐标/大小。

**可被引用的几何值**（局部与跨宾语均可用）：

- `tx`、`ty`、`tw`、`th`：**原始**（未移动）的 x/y/width/height。
- `nx`、`ny`、`nw`、`nh`：**移动后**的 x/y/width/height（应用 sx/ox 之后）。
- 跨宾语引用写法：`cite_name.tx`、`cite_name.nx` 等。

注意：

- `sx/sy/sw/sh` 与 `ox/oy/ow/oh` **仅内部计算使用，不对外发布**，不能作为跨宾语引用（外部应使用 `tx/nx` 等）。
- 当 `nx/ny/nw/nh` 无法正常计算（例如 `sx/ox` 引用了无法解析的值）时会直接**报错**并停止。

**产生宾语自身的坐标/大小**：各 info 还提供各自的 `gset/gsetsize` 字段（作用于产生宾语，发生在标记宾语自身调整之后）：

- `object_info`：`gset`、`gsetsize`
- `inadd_info`：`inaddgset`、`inaddgsetsize`
- `mtext_info`：`mgset`、`mgsetsize`
- `building_info`：`detectgset`/`detectgsetsize`、`addgset`/`addgsetsize`
- `multiAdd`/`multiRemove`/`multiText`/`teamDetect`/`numDetect`/`tree`：`gset`、`gsetsize`（二维数组，按组/分支分别设置）


`gset/gsetsize`（及各前缀版本）都是数字数组类型，因此同样遵循“数组元素允许是引用表达式”的规则，例如 `gset="{sx} 0"` 会被延迟解析。

产生宾语最终坐标/大小的顺序：先应用 `gset/gsetsize`（或当前标记宾体的移动后坐标/大小），再叠加 `offset/offsetsize`。
...
offsetsize：可选，数字二维数组，默认"0 0"，表示不同分支标记宾语大小改变。当只有一个坐标时，该偏移对所有分支均适用。不同分支之间用逗号隔开，一个分支有x和y两个坐标，中间空格隔开。填写例子"0 40,40 0"。

gset：可选，数字二维数组，默认当前坐标，表示不同分支宾语坐标设置。填写例子"0 -20,-20 0"。

gsetsize：可选，数字二维数组，默认当前大小，表示不同分支宾语大小设置。填写例子"0 40,40 0"。
...
gset：可选，数字数组，默认当前坐标，更换产生宾语坐标。

gsetsize：可选，数字数组，默认当前大小，更换产生宾语大小。
...
inaddgset: 当isinadd为是时可选，数字数组，默认当前坐标，更换产生宾语坐标。

inaddgsetsize: 当isinadd为是时可选，数字数组，默认当前大小，更换产生宾语大小。
...
mgset: 当ismtext为是时可选，数字数组，默认当前坐标，更换产生宾语坐标。

mgsetsize: 当ismtext为是时可选，数字数组，默认当前大小，更换产生宾语大小。
...
addgset：可选，数字数组，默认当前坐标，建筑添加的更换产生宾语坐标。

addgsetsize：可选，数字数组，默认当前大小，建筑添加的更换产生宾语大小。
...
detectgset: 可选，数字数组，默认当前坐标，建筑检测的更换产生宾语坐标。

detectgsetsize: 可选，数字数组，默认当前大小，建筑检测的更换产生宾语大小。
...
gset：可选，数字二维数组，默认当前坐标，表示不同阵营宾语坐标设置。填写例子"0 -20,-20 0"。

gsetsize：可选，数字二维数组，默认当前大小，表示不同阵营宾语大小设置。填写例子"0 40,40 0"。
...
id：可选，字符串数组，默认没有，检测宾语的 id。

alsoActivate：可选，字符串数组，默认没有，检测宾语的 alsoActivate。
...
gset：可选，数字二维数组，默认当前坐标，表示不同组宾语坐标设置。填写例子"0 -20,-20 0"。

gsetsize：可选，数字二维数组，默认当前大小，表示不同组宾语大小设置。填写例子"0 40,40 0"。
