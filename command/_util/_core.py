...
def question(language:str, condition:bool, tip:str, chosen_list:list, break_list:set, chosen_dict:dict, isauto = False, auto = None)->bool:
    ...
    while(condition):
        ...
        if chosen_set.issuperset([ispathsame]):
            ...
            standard_out(language, True, error_langstr)
    ...
    while(id_to_tobject_do):
        ...
        if id_to_tobject_do:
            ...
            if not istriggerauto:
                if question:
                    question(language, True, "The IDs are coincident and the mapping cannot be performed. And the map has not executed triggerauto. Would you want to rearrange ID?" + \
                            "|宾语ID发生重合，ID映射无法进行。且尚未自动化。是否重排ID？", ["y", "n"], ["n"], {}, 
                            isyes, "y")
                standard_out(language, isverbose, "IDs are arranging in the map...|铁锈地图宾语ID重排中...")
                rwmap.resetid()
            else:
                standard_out(language, True, f"The IDs are coincident and the mapping cannot be performed. And the map has executed triggerauto. Maybe object ID adjustment is required." + 
